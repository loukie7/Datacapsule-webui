          ...
          for (const sseMessage of messages) {
            ...
            if (event === 'chat_stream') {
              ...
              if (stepType !== 'final_answer') {
                // 推理阶段不更新streamedResponse，只更新reasoning
                if (hasReasoning) {
                ...
                // 最终答案阶段：流式显示答案
                if (hasAnswer) {
                  streamedResponse = data.answer;
                }
                if (hasReasoning) {
                  lastReasoning = data.reasoning;
                }
                
                console.log('最终答案流式更新 - Answer:', streamedResponse, 'Reasoning:', lastReasoning);
                
                yield {
                  type: 'stream',
                  content: streamedResponse, // 最终答案阶段显示答案内容
                  reasoning: lastReasoning,
                  stepType: stepType,
                  deltas: {
                    reasoning: data.reasoning_delta || '',
